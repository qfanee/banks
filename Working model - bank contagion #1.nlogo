...
to-report is-under-default-risk [b]
  let is-default false
  ask b [
...
to go
 ...
 set buffer .04
  ...
  ;; We should exclude initialized default turtles or turtles that have been already visited
  ask turtles with [((color = red) or (will-be-in-default = true) and (not member? self visited-agentset-banks))][ ; we should exclude default turtles that have been already visited
    set color red
  ...
  ; Sell loans granted by the banks that entered a default state.
;  ask turtles with [(color = red)][
;    buy-loans-of self ; Sell the loans accorded by the default bank to non-defaulted banks
;  ]
...
to initial-default-setup [ default-agent ]
  ask default-agent [
...
to exogenous-shock
  ask one-of turtles [
    initial-default-setup self
    let start-turtle one-of turtles
    print ("######################")
...
to biggest-size-exogenous-shock
  ask one-of turtles with [bank-size = banks-max-size] [
    initial-default-setup self
    let start-turtle one-of turtles
    print ("######################")
    let paths all-paths-from self
...
to smallest-size-exogenous-shock
  ask one-of turtles with [bank-size = banks-min-size] [
    initial-default-setup self
    let start-turtle one-of turtles
    print ("######################")
    let paths all-paths-from self
